...
// ==================== MODULO ESTUDIO ====================
wireSubnav("estudio-subnav","esub-",["materias","tareas","aula","sync"],{
  materias: loadMaterias, tareas: loadTareasFull, aula: loadAula, sync: loadSync
});
...
// -- Sincronizar con Aula Extendida (Moodle) --
async function getMoodleToken(){
  const { data } = await db.from("integrations").select("token").eq("provider","moodle").limit(1);
  return data && data[0] ? data[0].token : null;
}
...
async function loadSync(){
  ...
  if(!token){
    el.innerHTML = `
      <p style="font-size:13.5px;color:var(--muted);margin-bottom:12px;">Conecta tu cuenta de Aula Extendida una vez. Tu contrasena no se guarda: solo se usa para obtener un token, y ese token es lo unico que queda almacenado.</p>
      <div class="field-row-add">
        <input type="text" id="mo-user" placeholder="Usuario institucional">
        <input type="password" id="mo-pass" placeholder="Contrasena">
        <button class="btn primary" id="mo-connect" style="background:var(--estudio);color:#0A0D12;">Conectar</button>
      </div>
      <p class="auth-error" id="mo-error"></p>
    `;
    document.getElementById("mo-connect").onclick = async ()=>{
      const username = document.getElementById("mo-user").value.trim();
      const password = document.getElementById("mo-pass").value;
      const errEl = document.getElementById("mo-error");
      errEl.textContent = "";
      if(!username || !password){ errEl.textContent = "Completa usuario y contrasena."; return; }
      const btn = document.getElementById("mo-connect");
      btn.disabled = true; btn.textContent = "Conectando...";
      const { data, error } = await db.functions.invoke("moodle-sync", { body: { mode:"login", username, password } });
      btn.disabled = false; btn.textContent = "Conectar";
      if(error || (data && data.error)){ errEl.textContent = (data && data.error) || "No se pudo conectar."; return; }
      await db.from("integrations").upsert({ user_id: currentUser.id, provider:"moodle", token: data.token, updated_at: new Date().toISOString() }, { onConflict:"user_id,provider" });
      loadSync();
    };
    return;
  }
  ...
  el.innerHTML = `
    <p style="font-size:13.5px;color:var(--muted);margin-bottom:12px;">Cuenta de Aula Extendida conectada.</p>
    <button class="btn primary" id="mo-sync-btn" style="background:var(--estudio);color:#0A0D12;max-width:220px;">Sincronizar tareas ahora</button>
    <button class="btn ghost" id="mo-disconnect" style="max-width:160px;margin-top:8px;">Desconectar</button>
    <div id="sync-result" style="margin-top:14px;"></div>
  `;
  document.getElementById("mo-disconnect").onclick = async ()=>{
    await db.from("integrations").delete().eq("provider","moodle");
    loadSync();
  };
  document.getElementById("mo-sync-btn").onclick = async ()=>{
    const btn = document.getElementById("mo-sync-btn");
    const resultEl = document.getElementById("sync-result");
    btn.disabled = true; btn.textContent = "Sincronizando...";
    const { data, error } = await db.functions.invoke("moodle-sync", { body: { mode:"sync", token } });
    btn.disabled = false; btn.textContent = "Sincronizar tareas ahora";
    ...
    for(const t of (data.tasks||[])){
      ...
      await db.from("tasks").insert({ user_id: currentUser.id, subject_id, title: t.title, due_date: t.due_date, priority:"media" });
      existingKeys.add(key);
      creadas++;
    }
    resultEl.innerHTML = `<div class="empty-state">Sincronizado: ${(data.tasks||[]).length} tareas encontradas, ${creadas} nuevas agregadas.</div>`;
    loadTareas();
  };
}
